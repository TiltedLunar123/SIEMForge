...
- Tests for the converter paths that decide how a Sigma rule becomes a query. The condition parser now has cases for its malformed-input errors (unbalanced parenthesis, dangling operator, trailing tokens), and the render layer covers the `startswith` wildcard, the placeholder emitted for an undefined selection, the unknown-AST-node guard, single-clause OR groups across all three backends, and the Kibana keyword match. The `converters` package (`base.py`, `elastic.py`, `kibana.py`, `splunk.py`) is now fully covered.
...
- The test suite no longer logs `UnicodeDecodeError` warnings on Windows. The converter CLI tests read the tool's UTF-8 output through the default cp1252 codec; they now decode as UTF-8 with `errors="replace"`, the same way the rest of the suite does.
